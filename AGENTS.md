...
- **nutrient-dws / dws-viewer-api** — Upload documents to Nutrient cloud storage and mint browser viewer session JWTs via the DWS Viewer API. Use for cloud-backed document viewing, annotation, forms, and signing without running server infrastructure. NOT for the self-hosted Web SDK (`nutrient-web-sdk`) or self-hosted persistence (`nutrient-document-engine`).
