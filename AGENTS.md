...
- **nutrient-dws / remediate-pdf** — Remediate existing PDFs: auto-tag with PDF/UA semantic structure (headings, lists, tables, reading order) via the Nutrient DWS Accessibility API (Python script via `uv`; auto-tag only — verify tagged output with the make-pdf skill's `verify-pdf.py`).
