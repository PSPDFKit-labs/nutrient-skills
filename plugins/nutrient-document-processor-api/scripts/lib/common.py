...
def create_client():
    """Create and return a NutrientClient using the NUTRIENT_DWS_API_KEY env var."""
    api_key = os.environ.get("NUTRIENT_DWS_API_KEY")
    if not api_key:
        raise RuntimeError(
            "NUTRIENT_DWS_API_KEY is not set. Export it before running these scripts."
        )
    try:
