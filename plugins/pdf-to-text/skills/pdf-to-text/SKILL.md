...
name: pdf-to-text
description: >-
  Convert PDF content to layout-preserving plain text. Use this skill for tables,
  invoices, columns, or text-only tools that need whitespace and alignment. Use
  Standard for PDFs with selectable text and Vision for scans or image-only pages.
  Use pdf-to-markdown instead when headings, lists, and Markdown tables matter.
license: Proprietary
---
...
- Use `pdf-to-markdown` when headings, lists, and tables matter more than whitespace and column alignment.
- Do not render PDF pages as images to extract selectable text. Use Vision for scans and image-only PDFs.

# PDF to Text

Convert PDFs to plain text while preserving columns, indentation, and table alignment.

## Related skills

- **`pdf-to-markdown`** — preserves headings, lists, tables, and reading order as Markdown.
...
Omit `OUTPUT.txt` to write text to stdout.
...
Vision handles scans, photographs, handwriting, formulas, and complex layouts. With `pdf-to-text`, Vision writes structured JSON rather than plain text. Use `pdf-to-markdown --vision` when you want Markdown.
